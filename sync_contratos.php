<?php
    ...
    $serverName = "201.184.98.74\SQLEXPRESS, 1433";
    ...
    if((isset($_POST["dia"]) && isset($_POST["mes"])) || isset($_POST["contrato"]) || isset($_POST["nomina"]))
    {

        if(isset($_POST["contrato"]))
        {
            $contrato_search = $_POST["contrato"];
            $filtro = " AND t0.Contrato = '$contrato_search'";
        }
        else if(isset($_POST["nomina"]))
        {
            $nomina_search = $_POST["nomina"];
            $filtro = " AND t0.nomina = '$nomina_search'";
        }
        else
        {
            $dia_search = $_POST["dia"];
            $mes_search = $_POST["mes"];
            $filtro = " AND t0.fecha_creacion_contrato > DATEFROMPARTS (2021, $mes_search, $dia_search)";
        }

        $sql = "SELECT t0.Contrato as numero_contrato,CASE t0.Estado WHEN 1 THEN 'ACTIVO' ELSE 'RETIRADO' END AS estado_contrato, 
            CASE t0.Id_empresa WHEN 2 THEN 'HQ5 S.A.S' WHEN 5 THEN 'UT COLTEMP-HQ5' WHEN 4 THEN 'TECNOGESTION FD S.A.S' WHEN 3 THEN 'TEMPOENLACE S.A.S' END AS est,t1.Tipo_Documento,
            ...
            WHERE /*t0.Nomina='194' AND */ t0.Id_empresa=t3.Id_empresa $filtro
        ...
        while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
            ...
            $url = "https://creator.zoho.com/api/v2/hq5colombia/contratos-n-mina/form/Contrato";
            ...
            $Pension = str_replace(" ","",$row["afp"]);
            $Cesantias = str_replace(" ","",$row["afp"]);
            ...
            $Salud_Descontada = number_format($row["Salud_descontada"],0);
            $Salud_Prepagada_poliza_seguros = number_format($row["Salud_prepagada"],0);
            ...
            $departamento_contrato = (isset($result_dpto[0]["id"])) ? $result_dpto[0]["id"] : 3960020000000081123;

            $result_city = $db->sql_seleccionar("SELECT * FROM maestro_ciudad WHERE id_ciudad_fisoft=".$row["Ciudad"]);
            $ciudad_contrato = (isset($result_city[0]["id"])) ? $result_city[0]["id"] : 3960020000000085019;

            $result_banco = $db->sql_seleccionar("SELECT * FROM bancos WHERE descripcion='$Bancos'");
            $Bancos = (isset($result_banco[0]["id"])) ? $result_banco[0]["id"] : 3960020000000313003;

            $result_pension = $db->sql_seleccionar("SELECT * FROM fondos WHERE texto='$Pension'");
            $Pension = (isset($result_pension[0]["ID"])) ? $result_pension[0]["ID"] : "null";

            $result_cesantia = $db->sql_seleccionar("SELECT * FROM fondos WHERE texto='$Cesantias'");
            $Cesantias = (isset($result_cesantia[0]["ID"])) ? $result_cesantia[0]["ID"] : "null";

            $result_salud = $db->sql_seleccionar("SELECT * FROM fondos WHERE texto='$Salud'");
            $Salud = (isset($result_salud[0]["ID"])) ? $result_salud[0]["ID"] : "null";

            $result_caja = $db->sql_seleccionar("SELECT * FROM fondos WHERE texto='$Caja_Compensacion'");
            $Caja_Compensacion = (isset($result_caja[0]["ID"])) ? $result_caja[0]["ID"] : "null";

            $result_arl = $db->sql_seleccionar("SELECT * FROM fondos WHERE texto='$ARL'");
            $ARL = (isset($result_arl[0]["ID"])) ? $result_arl[0]["ID"] : "null";

            $tipoCotizante = $db->sql_seleccionar("SELECT * FROM maestro_cotizante WHERE cotizante='$Tipo_Cotizante'");
            $Tipo_Cotizante = (isset($tipoCotizante[0]["id"])) ? $tipoCotizante[0]["id"] : "null";

            $result_conceptoCotizante = $db->sql_seleccionar("SELECT * FROM maestro_conceptocotizante WHERE concepto='$Concepto_Cotizante'");
            $Concepto_Cotizante = (isset($result_conceptoCotizante[0]["id"])) ? $result_conceptoCotizante[0]["id"] : "null";
            ...
            $resultVariableToken = $db->sql_seleccionar("SELECT * FROM app_variables WHERE variable='TokenOAuth_Zoho'");
            $token = (isset($resultVariableToken[0]["valor_texto"])) ? $resultVariableToken[0]["valor_texto"] : "null";

            $curl = curl_init();

            curl_setopt_array($curl, array(
            CURLOPT_URL => "https://creator.zoho.com/api/v2/hq5colombia/contratos-n-mina/form/Contrato",
            ...
            $info = "Contrato: ".$Numero_de_Contrato."<br/>Documento:".$Numero_de_documento;

            if(isset($data_json->code))
            {
                if($data_json->code == 3002)
                {
                    unset($data["Numero_de_Contrato"]);
                    $postdataupdate = json_encode( array("criteria" => "(Numero_de_Contrato == \"$Numero_de_Contrato\")",   "data" => $data) );
                    
                    $curl = curl_init();
    
                    curl_setopt_array($curl, array(
                        CURLOPT_URL => "https://creator.zoho.com/api/v2/hq5colombia/contratos-n-mina/report/Contrato_Report",
                        CURLOPT_RETURNTRANSFER => true,
                        CURLOPT_ENCODING => "",
                        CURLOPT_MAXREDIRS => 10,
                        CURLOPT_TIMEOUT => 0,
                        ...
                        CURLOPT_POSTFIELDS => $postdataupdate,
                        CURLOPT_HTTPHEADER => array(
                            "Authorization: Zoho-oauthtoken $token"
                        ),
                    ));
                    ...
                    $valores_response = array("info" => $info,"status" => "<span class='text-info'>".$response."</span>");
                    
                }
                else if($data_json->code == 3000) $valores_response = array("info" => $info,"status" => "<span class='text-success'>".$response."</span>");
                else $valores_response = array("info" => $info,"status" => "<span class='text-danger'>".$response."</span>");
            }
            else $valores_response = array("info" => $info,"status" => "<span class='text-danger'>".$response."</span>");
...
<html lang="en">
  <head>
    ...
    <title>Sincronizar Contratos</title>
  </head>
  <body>
        ...
        <div class="row">
            <div class="col-8 offset-2 py-4">
                ...
                <?php if(count($results_inyection) == 0)
                {
                    ...
                    <form action="#" method="POST">
                        <div class="row text-center p-2">
                            <div class="col-12">Por Nomina</div>
                            <div class="col-8">
                                <label for="nomina">Numero de Nomina</label>
                                <input type="number" name='nomina' class="form-control" placeholder='' min='0' max='999999' required> 
                            </div>
                            <div class="col-4">
                                <br>
                                <button class='btn btn-success sincronizar'>Sincronizar Por Nomina</button>
                            </div>
                        </div>
                    </form>
                    <?php
